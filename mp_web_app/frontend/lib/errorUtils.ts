export function extractApiErrorDetails(error: any): string {
  ...
  const translate = (msg: string): string => {
    ...
    if (/Invalid image format/i.test(msg))
      return "Невалиден формат на изображение. Разрешени формати: JPG, JPEG, PNG, GIF, WEBP";
    if (/Invalid members list file type/i.test(msg))
      return "Невалиден тип файл за списък с членове. Разрешен тип: .csv";
