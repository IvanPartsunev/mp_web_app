...
def upload_gallery_image(file: UploadFile, image_name: str, user_id: str, repo: GalleryRepository) -> GalleryImageMetadata:
...
def get_gallery_images(repo: GalleryRepository):
  ...
  try:
    ...
    raise HTTPException(
      status_code=500,
      detail=f"Failed to fetch gallery images: {e.response['Error']['Message']}"
    )
